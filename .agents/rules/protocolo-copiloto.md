...
Seu escopo, metas de performance (SLAs) e restrições estão definidos no arquivo `markdowns-ativos/prompt-mestre.md`. Aja estritamente de acordo com ele. Você não precisa ler este arquivo a cada interação, mas DEVE consultá-lo silenciosamente caso perca o contexto, sinta que a conversa ficou muito longa, ou ao iniciar uma nova etapa do projeto.
...
* **Gatilho A: Alteração de Dados ou Features (Execução de Script)**
  ...
  * *Ação Obrigatória (Autônoma):* Atualize *SEMPRE* o `markdowns-ativos/logs.md` (com o state tracking). **E, EXCLUSIVAMENTE SE** houver criação de novas variáveis ou mudança semântica/matemática delas, atualize o `markdowns-ativos/dicionario-features.md` (com a definição matemática viva). Não peça permissão ao usuário, apenas faça.
...
* **Gatilho C: Unidade Lógica Concluída (Commit)**
  ...
  * *Referência:* **Consulte obrigatoriamente a regra `padrao-commits.md` para realizar o commit de acordo com o padrão do projeto.**

## 5. Transição de Fases (O Marco Oficial)

Quando concluirmos 100% das tarefas de uma etapa definida no arquivo `contexto/PLANO.md` (ex: Fim da Limpeza, Fim do Pipeline de Features), envie ao usuário EXATAMENTE esta mensagem isolada no chat:
